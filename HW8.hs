...
firsts re =  create (lang2 re)

top ::  [[symbol]] -> [symbol]
top [[]] = []
top (x:xs)  = interleave [head x] []

create:: [[symbol]] -> [symbol]
create list =  if length(list) == 1 then top (list) else if (length(list)==0) then [] else interleave (top list) (create (tail list))

--modified version of lang--
lang2 :: RE symbol -> [[symbol]]
lang2 (RSym s) = [[s]]
lang2 REps = [[]]
lang2 RZero = []
lang2 (RAlt a b) = interleave (lang2 a) (lang2 b)
lang2 (RSeq a b) | null (lang2 b) = []
lang2 (RSeq a b) = foldr (\s -> interleave (map (s ++) (lang2 b))) [] (lang2 a)
lang2 (RStar a)  = lang2 a
lang2 (RPlus a)  = lang2 a
